export const PROFILE_ID = 'governed-mission-v1';
const PREFIX = 'AVP1';
...
export function decode(wire, profileId = PROFILE_ID) {
  ...
  if (first < 0 || second < 0 || wire.slice(0, first) !== PREFIX) {
    throw new CodecError('invalid AVP1 wire header');
  }
  const actualProfile = wire.slice(first + 1, second);
  if (actualProfile !== profileId) {
    throw new ContextMismatchError(`profile mismatch: expected ${profileId}, got ${actualProfile}`);
  }
  let packed;
  try {
    packed = JSON.parse(wire.slice(second + 1));
  } catch {
    throw new CodecError('invalid AVP1 JSON body');
  }
