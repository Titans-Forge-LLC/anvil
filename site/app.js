(function () {
  'use strict';
  const { PROFILE_ID, canonicalJson, decode, encode, sha256 } = window.ANVIL_AVP1;
  const defaultMission = {
    authority: {
      forbids: [{ kind: 'publish', target: '*' }],
      permits: [{ kind: 'read', target: 'source' }, { kind: 'write', target: 'staging' }],
      requirements: [
        { effect: { kind: 'write', target: 'staging' }, gate: 'fox_pass', kind: 'gate' },
        { effect: { kind: 'publish', target: 'posts' }, kind: 'approval', mode: 'exact_text', target: 'human' }
      ]
    },
    inputs: [{ name: 'source', ref: 'sha256:synthetic-source-pack', type: 'Artifact<SourcePack>' }],
    mission: 'governed_social',
    outputs: [{ name: 'staged_posts', type: 'Artifact<StagedPosts>' }],
    steps: [{
  ...
  document.querySelector('#encode').addEventListener('click', () => run());
  document.querySelector('#wrong-context').addEventListener('click', () => run('wrong-profile'));
